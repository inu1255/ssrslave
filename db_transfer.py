...
class TransferBase(object):
	...
	def del_server_out_of_bound_safe(self, last_rows, rows):
		...
		for row in rows:
			try:
				allow = switchrule.isTurnOn(row) and row['enable'] and row['u'] + row['d'] < row['transfer_enable']
			except Exception as e:
				allow = False
	...
	def update_all_user(self, dt_transfer):
		...
		for id in dt_transfer.keys():
			...
			if transfer[0] + transfer[1] < update_trs and id not in self.force_update_transfer:
				...
				continue
			data = self.request("/traffic",{"port":id,"u":transfer[0],"d":transfer[1]})
			if data:
...
class Dbv3Transfer(DbTransfer):
	def __init__(self):
		...
		if self.update_node_state:
			...
			self.from_user = " user"
			if get_config().API_INTERFACE == 'ssrslave':
				self.key_list += ['obfs', 'protocol', 'protocol_param', 'speed_limit_per_user']
				self.from_user = " user where node_id=" + str(self.cfg["node_id"])
	...
	def pull_db_users(self, conn):
		...
		try:
			rows = []
			cur.execute("SELECT " + ','.join(keys) + " FROM" + self.from_user)
			for r in cur.fetchall():
				d = {}
				for column in range(len(keys)):
					d[keys[column]] = r[column]
	...
	def load(self):
		import os
		lines = os.popen("cat /proc/loadavg | awk '{ print $1\" \"$2\" \"$3 }'").readlines()
		if lines:
			return lines[0]
		return ""
